...
const parseHarnessJson = express.json({ limit: '10mb' });
harness.post('/api/e2e/seed-message', parseHarnessJson, (request, response) => {
  const { conversationId, author, body, status } = request.body ?? {};
  const message = seedRepository.createSharedMessage(author, body, status ?? 'completed', conversationId);
  publishRealtimeMessagesEvent(message.conversationId);
  response.status(201).json({ message });
});
harness.post('/api/e2e/update-message', parseHarnessJson, (request, response) => {
  const { id, body, status } = request.body ?? {};
  const message = seedRepository.updateSharedMessage(id, { body, status });
  if (message) publishRealtimeMessagesEvent(message.conversationId);
  response.status(200).json({ message });
});
harness.post('/api/work-items/:id/execute', parseHarnessJson, (request, response) => {
  const item = seedRepository.get(request.params.id);
  if (!item) return response.status(404).json({ error: 'Work item not found.' });
  if (request.body?.fail || item.title.startsWith('E2E rejected run')) return response.status(503).json({ error: 'Agent service is unavailable.' });
  ...
  publishRealtimeWorkItemEvent(item.id);
  publishRealtimeMessagesEvent(conversation.id);
  response.json({ run, runs: [run], classification, conversation, activity });
});
harness.post('/api/e2e/complete-run', parseHarnessJson, (request, response) => {
  ...
  publishRealtimeWorkItemEvent(run.workItemId);
  if (message) publishRealtimeMessagesEvent(message.conversationId);
...
attachRealtimeServer(server, { handleRequest: createApplicationSocketHandler(harness) });
