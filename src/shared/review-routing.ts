...
/** Compiler-derived facts used by routing. Kept structural so browser and
 * server callers can supply the same reading without importing one another. */
export interface ReviewBlockAnalysis {
  effect: string;
  score: number;
  hazards: string[];
}
...
export interface ReviewRoutingContext {
  /** Retained for rolling compatibility with callers opened before Review
   * Director. Test-only work now follows one policy in every review. */
...
function readsAsNonRunning(analysis: ReviewBlockAnalysis | null): boolean {
...
const OBLIGATION_PROOFS: Record<
  string,
  (decision: ReviewDecision, analysis: ReviewBlockAnalysis | null) => Pick<ReviewObligation, 'outcome' | 'evidence'> | null
> = {
...
export function settleObligations(
  ...
  analysis: ReviewBlockAnalysis | null = null,
): ReviewObligation[] {
...
export function routeReviewBlock(
  ...
  // Tests ship to nobody, so they never outrank production code. They are not
  // blindly settled either: a bounded delegated turn checks for weakened or
  // misleading coverage, then closes the decision only when it is confident.
  if (decision.changeType === 'test_only') {
    return { tier: 'T1', reason: 'Test-only — delegated after production changes.', autoSettled: false };
  }
