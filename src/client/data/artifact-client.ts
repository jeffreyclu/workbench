...
export const artifactClient = {
  ...
  listArtifacts: (view: 'published' | 'revoked' | 'all' | 'favorites' = 'published') => request<{ artifacts: ArtifactSummary[]; counts: { published: number; revoked: number; favorited: number; openComments: number } }>(`/api/artifacts?view=${view}`),
  ...
  setArtifactFavorited: (id: string, favorited: boolean) => request<{ artifact: ArtifactSummary }>(`/api/artifacts/${id}/favorite`, { method: 'PATCH', body: JSON.stringify({ favorited }) }),
