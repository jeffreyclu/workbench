...
export function ComposerModelSelect({ executionProfile, provider, onChange, accountProfile, disabled = false }: {
  ...
  return <>
    ...
    {provider === 'palmyra'
      ? <select id={selectId} className="model-target" value="palmyra-x5" onChange={(event) => onChange(event.target.value as ComposerProfile)} disabled={disabled} title={availability.data?.palmyra?.reason ?? undefined}>
