...
function SourceConnectionCard({ connection }: { connection: BrokerConnection }) {
  ...
  const mcpConnect = useMutation({
    mutationFn: async () => {
      ...
      if (provider === 'figma' || provider === 'atlassian') {
        if (reconnecting && provider === 'atlassian') await api.disconnectSource('confluence');
        const { url } = await api.startManagedMcpOAuth(provider);
        return url;
      }
  ...
  const figmaScope = useQuery({ queryKey: ['figma-scope'], queryFn: api.getFigmaScope, enabled: provider === 'figma' && connected });
  ...
  const saveFigmaScope = useMutation({
    mutationFn: () => api.updateFigmaScope(figmaRoots.split('\n').map((root) => root.trim()).filter(Boolean)),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['figma-scope'] }),
  });
