...
import { fileLabel, parsePatch } from '../github-diff/logic.js';
import { useWorkspaceDiff } from './hooks.js';

function DiffSkeleton() {
  return <section className="github-diff" aria-label="Workspace changes loading" aria-busy="true">
...
export const WorkspaceDiffView = memo(function WorkspaceDiffView({ workItemId, isRunning }: { workItemId: string; isRunning: boolean }) {
  ...
  if (query.isError) return <section className="github-diff github-diff-error" aria-live="polite"><strong>Could not load local workspace changes.</strong><p>{query.error.message}</p></section>;
  if (!query.data) return null;
  const { diff } = query.data;

  return <section className="github-diff workspace-diff" aria-label="Current workspace changes">
    <header>
      <div><span className="github-diff-eyebrow"><FileDiff size={14} /> Review before push</span><h2>Current workspace changes</h2><small>{diff.branch} · {diff.changedFiles} files · <b>+{diff.additions}</b> <i>−{diff.deletions}</i></small><p>Uncommitted changes in this task’s workspace. This includes staged, unstaged, and untracked files.</p></div>
    ...
    {files.length === 0 ? <p className="muted">No uncommitted changes to review.</p> : <div className="github-diff-layout">
