...
export function parsePatch(patch: string): DiffLine[] {
  ...
  return patch.split('\n').map((text, index) => {
    ...
    // GitHub includes this patch annotation after a line with no trailing
    // newline. It is not source content and must not consume either counter.
    if (text === '\\ No newline at end of file') {
      return { key: `${index}:${text}`, kind: 'context' as const, oldLine: null, newLine: null, text };
    }
    if (text.startsWith('+')) {
