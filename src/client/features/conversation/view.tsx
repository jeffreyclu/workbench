  ...
  return (
    <main className={`shared-workspace ${railOpen ? 'rail-open' : ''} ${showingConversationStackOnly ? 'stack-only' : ''}`}>
      ...
      <section className="agent-console" aria-label="Shared agent workspace">
        ...
        <div className="mobile-chrome-controls"><button type="button" className={`mobile-chrome-toggle mobile-conversation-toggle icon-button${mobileHeaderOpen ? ' icon-button-active' : ''}`} onClick={() => setMobileHeaderOpen((open) => !open)} aria-label={mobileHeaderOpen ? 'Collapse conversation tray' : 'Expand conversation tray'} title={mobileHeaderOpen ? 'Collapse conversation tray' : 'Expand conversation tray'}><PanelTop size={16} /></button>{!mobileComposerOpen && <button type="button" className="mobile-chrome-toggle mobile-composer-toggle icon-button" onClick={() => setMobileComposerOpen(true)} aria-label="Expand composer" title="Expand composer"><SquarePen size={16} /></button>}</div>
