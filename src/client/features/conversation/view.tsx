  ...
  return (
    <main className={`shared-workspace ${railOpen ? 'rail-open' : ''} ${showingConversationStackOnly ? 'stack-only' : ''}`}>
      ...
      <section className="agent-console" aria-label="Shared agent workspace">
        ...
        {/* 81bb6f51-57d4-4778-aec4-8fb22e3ba617 LEGACY-AFFECTING: The existing conversation pane now shows query freshness and refetches its canonical record, messages, and linked task together. */}
        {conversationId && selectedConversation && <FreshnessControl updatedAt={Math.max(conversationDetail.dataUpdatedAt, messages.dataUpdatedAt, linkedWorkItem.dataUpdatedAt)} isRefreshing={conversationDetail.isFetching || messages.isFetching || linkedWorkItem.isFetching} onRefresh={() => void Promise.all([conversationDetail.refetch(), messages.refetch(), linkedWorkItemId ? linkedWorkItem.refetch() : Promise.resolve()])} />}
