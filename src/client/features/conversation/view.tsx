  ...
  return (
    <main className={`shared-workspace ${railOpen ? 'rail-open' : ''} ${showingConversationStackOnly ? 'stack-only' : ''}`}>
      ...
      <section className="agent-console" aria-label="Shared agent workspace">
        ...
        <div className="mobile-chrome-controls"><button type="button" className={`mobile-chrome-toggle mobile-conversation-toggle icon-button${mobileHeaderOpen ? ' icon-button-active' : ''}`} onClick={() => setMobileHeaderOpen((open) => !open)} aria-label={mobileHeaderOpen ? 'Collapse conversation header' : 'Expand conversation header'} title={mobileHeaderOpen ? 'Collapse conversation header' : 'Expand conversation header'}><PanelTop size={16} /></button><button type="button" className={`mobile-chrome-toggle mobile-composer-toggle icon-button${mobileComposerOpen ? ' icon-button-active' : ''}`} onClick={() => setMobileComposerOpen((open) => !open)} aria-label={mobileComposerOpen ? 'Collapse composer' : 'Expand composer'} title={mobileComposerOpen ? 'Collapse composer' : 'Expand composer'}><SquarePen size={16} /></button></div>
