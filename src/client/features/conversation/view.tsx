...
export function SharedWorkspace({ initialConversationId, onOpenTask, onSelectConversation, view, onViewChange }: { initialConversationId?: string | null; onOpenTask?: (taskId: string) => void; onSelectConversation?: (conversationId: string | null) => void; view?: 'active' | 'archive'; onViewChange?: (view: 'active' | 'archive') => void }) {
  ...
  const changesAvailability = useConversationChangesAvailability(
    linkedWorkItem.data?.item?.id ?? null,
  ...
  return (
    <main className={`shared-workspace ${railOpen ? 'rail-open' : ''} ${showingConversationStackOnly ? 'stack-only' : ''}`}>
      ...
      <section className="agent-console" aria-label="Shared agent workspace">
        ...
        {linkedWorkItem.data?.item && <div className="thread-filter-bar"><div className="conversation-surface-tabs" role="group" aria-label="Conversation review layout"><button type="button" aria-pressed={conversationSurface === 'messages'} onClick={() => setConversationSurface('messages')}>Conversation</button><button type="button" aria-pressed={conversationSurface === 'changes'} onClick={() => setConversationSurface('changes')} disabled={!changesAvailability.hasChanges} title={changesAvailability.hasChanges ? 'Review changes' : changesAvailability.isLoading ? 'Checking for changes…' : 'No changes to review'}><FileDiff size={13} /> Changes</button></div><TaskClassificationSelect itemId={linkedWorkItem.data.item.id} kind={linkedWorkItem.data.item.classificationKind} /><button type="button" className={`icon-button${linkedWorkItem.data.item.status === 'pinned' ? ' icon-button-active' : ''}`} onClick={() => toggleLinkedTaskPin.mutate()} disabled={toggleLinkedTaskPin.isPending} aria-pressed={linkedWorkItem.data.item.status === 'pinned'} aria-label={linkedWorkItem.data.item.status === 'pinned' ? 'Bring back' : 'Put a pin in it'} title={linkedWorkItem.data.item.status === 'pinned' ? 'Bring back' : 'Put a pin in it'}><Pin size={13} fill={linkedWorkItem.data.item.status === 'pinned' ? 'currentColor' : 'none'} /></button></div>}
        ...
        {conversationSurface === 'changes' && linkedWorkItem.data && <div className="conversation-changes" aria-label="Task changes"><WorkspaceDiffView workItemId={linkedWorkItem.data.item.id} isRunning={linkedWorkItem.data.runs.some((run) => run.status === 'queued' || run.status === 'running')} />{linkedPullRequestUrl && <GitHubDiffView sourceUrl={linkedWorkItem.data.item.sourceUrl} references={linkedWorkItem.data.references} />}</div>}
        </div>
      </section>
