...
import { ConversationComposerSkeleton, ConversationRailSkeleton, ConversationSearchResultSkeleton, ConversationThreadSkeleton, Skeleton } from '../../components/skeleton/skeleton';
import { Toaster } from '../../components/toast/toast';
import { toast, toastError } from '../../state/toast-store';
import { Tabs } from '../../components/tabs/tabs';
import { ClassificationKindDisclosure, nextActionSummary, SortableQueueItem as TaskQueueItem, TaskClassificationSelect } from '../queue';
import { AgentMessageBody, LiveRunOutput, splitBodyAtInterjections, type AgentMessageInterjection } from '../../components/agent-message/agent-message';
...
export function SharedWorkspace({ initialConversationId, initialStackOnly = false, onOpenTask, onSelectConversation, view, onViewChange }: { initialConversationId?: string | null; initialStackOnly?: boolean; onOpenTask?: (taskId: string) => void; onSelectConversation?: (conversationId: string | null) => void; view?: 'active' | 'archive'; onViewChange?: (view: 'active' | 'archive') => void }) {
  ...
  // immediate per-window selection during the round-trip between rail cards.
  ...
  const updateConversationPreferences = useMutation({
    ...
    onSuccess: async ({ conversation }, { conversationId: updatedConversationId, sequence }) => {
      ...
      if (conversationIdRef.current === updatedConversationId && sequence === preferenceMutationSequence.current) {
        const selection = composerSelectionFromConversation(conversation);
        composerSelectionsRef.current.set(updatedConversationId, selection);
  ...
  useEffect(() => {
    ...
    setFiles([]);
    const selection = pendingComposerSelectionRef.current ?? (conversationId ? composerSelectionsRef.current.get(conversationId) : null) ?? defaultComposerSelection();
    if (conversationId) composerSelectionsRef.current.set(conversationId, selection);
    setComposerSelection(selection);
  ...
  function openConversation(nextConversationId: string, source?: Pick<SharedConversation, 'preferredExecutionProfile' | 'preferredAccountProfile' | 'preferredDispatchTarget' | 'preferredAiProvider'>) {
    ...
    pendingComposerSelectionRef.current = source ? composerSelectionFromConversation(source) : composerSelectionsRef.current.get(nextConversationId) ?? null;
  ...
  function updateComposerPreferences(updates: Partial<ComposerSelection>) {
    ...
    setComposerSelection((current) => {
      ...
      composerSelectionsRef.current.set(targetConversationId, next);
      return next;
    });
  ...
  useEffect(() => {
    ...
    const updatePhoneChrome = () => {
      setIsPhoneChrome(media.matches);
      if (!media.matches) {
    ...
    updatePhoneChrome();
    media.addEventListener('change', updatePhoneChrome);
    return () => media.removeEventListener('change', updatePhoneChrome);
  }, []);
  ...
  // count changes.
  const conversationStackShape = conversationStackRows.map((row) => row.type === 'header' ? `h:${row.id}:${row.count}` : `i:${row.id}:${row.state ?? 'finished'}`).join('|');
  useEffect(() => {
  ...
  const conversationUpdatedAt = Math.max(conversationDetail.dataUpdatedAt, messages.dataUpdatedAt, linkedWorkItem.dataUpdatedAt);
  const conversationIsRefreshing = conversationDetail.isFetching || messages.isFetching || linkedWorkItem.isFetching;
  const refreshConversation = () => void Promise.all([conversationDetail.refetch(), messages.refetch(), linkedWorkItemId ? linkedWorkItem.refetch() : Promise.resolve()]);
  ...
  const githubCandidateUrls = useMemo(() => pullRequestUrls([
    ...(linkedWorkItem.data?.item?.sourceUrl ? [linkedWorkItem.data.item.sourceUrl] : []),
    ...(linkedWorkItem.data?.references ?? []).map((reference) => reference.url),
  ...
  const conversationRenderRows = useMemo(() => {
    ...
    for (let i = 0; i < renderedConversationMessages.length; i++) {
      ...
      if (next && isAgent(message) && isAgent(next) && message.author !== next.author && message.dispatchGroupId === next.dispatchGroupId) {
  ...
  const renderManualExecutionKindControl = () => selectedConversation && !selectedConversation.workItemId && (
    <ClassificationKindDisclosure kind={manualConversationExecutionKind} label="Execution type" pending={setManualConversationExecutionKind.isPending} onChange={(kind) => { if (manualConversationExecutionMessage) setManualConversationExecutionKind.mutate(kind); else if (conversationId) setNewConversationExecutionKinds((current) => ({ ...current, [conversationId]: kind })); }} />
  );
  ...
  const retryReply = useMutation<unknown, Error, SharedMessage>({
    ...
    onMutate: (message) => {
      // Starting a retry can take a few seconds while a canceled provider run
      // settles. A per-message state gives immediate feedback without making
      // independent agent replies wait on each other.
    ...
    onSettled: (_result, _error, message) => {
  ...
  return (
    <main className={`shared-workspace ${railOpen ? 'rail-open' : ''} ${showingConversationStackOnly ? 'stack-only' : ''}`}>
      ...
      <section className="agent-console" aria-label="Shared agent workspace">
        ...
        {conversationId && selectedConversation && <div className="conversation-freshness-control"><FreshnessControl compact updatedAt={conversationUpdatedAt} isRefreshing={conversationIsRefreshing} onRefresh={refreshConversation} /></div>}
