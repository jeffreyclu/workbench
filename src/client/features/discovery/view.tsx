...
// 537d343f-e2e9-47b3-a11f-be372345f508 LEGACY-AFFECTING: Existing inbox callers now see a retry state instead of an empty inbox when its query fails.
  ...
  return <section className="discovery-workspace">
    ...
    <div className="discovery-list">
      ...
      {inboxView === 'reviewed' ? inbox.data?.candidates.map((candidate) => {
        ...
        return <article className="discovery-card reviewed" key={candidate.id}><div className="discovery-source"><label><span>{candidate.provider}</span><em className={`decision-${candidate.status}`}>{candidate.status}</em></label><time>{new Date(candidate.updatedAt).toLocaleString()}</time></div><h3>{candidate.title}</h3>{candidate.description && <p>{candidate.description}</p>}<div className="discovery-actions">{candidate.sourceUrl && <a className="button secondary compact" href={candidate.sourceUrl} target="_blank" rel="noreferrer"><ArrowUpRight size={13} /> Source</a>}{candidate.workItemId && <button className="button secondary compact" onClick={() => onOpenTask(candidate.workItemId!)}>Open task</button>}{(candidate.status === 'dismissed' || candidate.status === 'snoozed') && <button className="button primary compact" disabled={isRestoring} onClick={() => restore.mutate(candidate.id)}>{isRestoring ? <><LoaderCircle className="spin" size={13} /> Restoring…</> : <><RefreshCw size={13} /> Restore to inbox</>}</button>}</div></article>;
      }) : inbox.data?.candidates.map((candidate) => <DiscoveryCard key={candidate.id} candidate={candidate} selected={selected.has(candidate.id)} tasks={activeTasks.data?.items ?? []} pendingAction={pendingActionFor(candidate.id)}
...
function DiscoveryCard({ candidate, selected, tasks, pendingAction, onSelected, onResolve }: { candidate: DiscoveryCandidate; selected: boolean; tasks: WorkItem[]; pendingAction: 'convert' | 'dismiss' | 'snooze' | 'merge' | null; onSelected: (checked: boolean) => void; onResolve: (action: 'convert' | 'dismiss' | 'snooze' | 'merge', workItemId?: string) => void }) {
  ...
  const priorityLabel = candidate.relevance >= 4 ? 'Focus' : candidate.relevance >= 3 ? 'Priority' : candidate.relevance === 2 ? 'Focus' : null;
  return <article className={`discovery-card ${selected ? 'selected' : ''}`}>
    <div className="discovery-source"><label><input type="checkbox" checked={selected} onChange={(event) => onSelected(event.target.checked)} /><span>{candidate.provider}</span>{priorityLabel && <em>{priorityLabel}</em>}</label><time>{new Date(candidate.occurredAt ?? candidate.discoveredAt).toLocaleString()}</time></div>
