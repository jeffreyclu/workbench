  ...
  return (
    <section className={`detail-panel ${isExecutionActive ? 'execution-starting' : ''}`} aria-busy={isExecutionActive}>
      ...
      <details className="detail-section task-collapsible workspace-review-section">
        ...
        <div className="task-collapsible-content">
          <WorkspaceDiffView scope={{ workItemId: item.id }} activeWorkspacePaths={detail.data.runs.filter((run) => run.status === 'queued' || run.status === 'running').flatMap((run) => run.resolvedWorkspace ? [run.resolvedWorkspace] : [])} taskIntent={{ title: item.title, description: item.description }} pullRequestUrlCandidates={references.map((reference) => reference.url)} />
        </div>
      </details>
