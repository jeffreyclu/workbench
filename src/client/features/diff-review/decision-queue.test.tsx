...
describe('diff review decision queue', () => {
  ...
  it('puts delegated decisions at the back and says they are waiting on the supervisor', () => {
    ...
    // The claimed decision started first in Director priority. While its turn
    // is running it renders after every unclaimed decision, including settled
    // history, so it cannot interrupt the human queue.
    const buttons = screen.getAllByRole('button');
    expect(buttons.map((button) => button.querySelector('b')?.textContent)).toEqual(['2', '3', '4', '1']);
    expect(buttons[3]).toHaveAccessibleName(/awaiting delegated review/);
    expect(buttons[3].className).toContain('delegating');
    // A settled decision is never owed a turn, so it must not be marked as one.
    expect(buttons[0]).not.toHaveAccessibleName(/awaiting delegated review/);
  });
  ...
  it('marks a completed delegation that explicitly asks for human review', () => {
    ...
    const decisionButton = screen.getAllByRole('button')[0];
    expect(decisionButton.className).toContain('escalated');
    expect(decisionButton).toHaveAccessibleName(/delegated review needs human review.*call sites outside this diff/i);
  });
});
