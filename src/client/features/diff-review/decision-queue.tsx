  ...
  return <div className="diff-review-queue-region">
    <nav className="diff-review-decision-queue" aria-label="Review decision queue">
      ...
      <ol>{decisions.map((decision) => <li key={decision.id}>
        <button type="button" className={`state-${decision.state ?? 'pending'}${decision.state === null ? '' : ' settled'}${decision.id === selectedId ? ' selected' : ''}`} aria-current={decision.id === selectedId ? 'step' : undefined} onClick={() => onSelect(decision.id)}>
