          ...
          {data.byAgent.length === 0 && data.byKind.length === 0 && data.tokenUsageByModel.length === 0 && data.cursing.messagesAnalyzed === 0 ? (
            ...
            <>
              <div className="insight-overall-row">
                ...
                <div className="insight-overall-stat"><span className="eyebrow">Median task cycle <InfoTooltip>Median of (completed_at − created_at) across all tasks completed in this window. The midpoint of that duration list, so it's not skewed by a few very slow or very fast tasks.</InfoTooltip></span><strong>{formatDuration(data.medianTaskCycleMs ?? null)}</strong><small>Typical time from task creation to completion.</small></div>
