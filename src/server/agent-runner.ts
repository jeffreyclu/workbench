...
Persistent processes: never trap a turn inside a foreground dev server, file watcher, service monitor, or command documented to run until Ctrl+C. When the current request explicitly requires a temporary service for dependent work, start it through Workbench's start_managed_command tool with a stable service key, inspect it with waitMs 0 until its saved output proves readiness, run the dependent work, and stop it with stop_managed_command before the turn ends. Never use raw shell backgrounding or nohup. Finite benchmarks, builds, and tests expected to exceed 90 seconds also use start_managed_command with a stable key and poll inspect_managed_command until terminal. Do not add tail or tee; Workbench captures stdout and stderr directly. Output and status persist to disk, so a resumed provider must inspect and continue the saved job rather than restart from zero.
