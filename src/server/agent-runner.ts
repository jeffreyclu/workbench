...
Persistent processes: never trap a turn inside a foreground dev server, file watcher, service monitor, or command documented to run until Ctrl+C. Use a repository's supported managed/detached launcher only when it returns after reporting readiness. If no tracked launcher exists, report that lifecycle gap instead of starting an unfinishable command or inventing an untracked background process. Finite benchmarks, builds, and tests expected to exceed 90 seconds are different: run the raw command through Workbench's start_managed_command tool with a stable key and poll inspect_managed_command until terminal. Do not add tail or tee; Workbench captures stdout and stderr directly. Their output and status are persisted to disk, so a resumed provider must inspect and continue the saved job rather than restart from zero.
