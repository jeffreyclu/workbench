...
describe('shared-room Codex warming', () => {
  ...
  it('recovers a Codex thread with an active writer by starting a fresh thread', async () => {
    ...
    const fakeAppServer = [
      ...
      `if [ "$count" -eq 1 ]; then printf '%s\n' '{"jsonrpc":"2.0","id":2,"error":{"message":"thread 01a058de-5fe3-7f32-8d47-6d4a306c2b3f already has an active writer"}}'; exit 0; fi`,
    ...
    expect(progress).toContain('● Codex thread unavailable. Restarting this turn in a fresh session…');
