...
export const EXTERNAL_ACTION_COMMANDS: readonly AuthorizationRule[] = [
  ...
  { id: 'pr_review', description: 'Submit a review, approval, change request, or comment on the named pull request', requiredExecutables: ['gh'], pattern: /\b(?:approve|comment\s+on|post\s+(?:a\s+)?comment\s+(?:on|to)|request\s+changes\s+(?:on|to))\s+(?:the\s+)?(?:github\s+)?(?:pull request|pr)\b|\b(?:submit|post)\s+(?:the\s+|a\s+)?(?:review|approval|change request|pr comment)\b/i },
