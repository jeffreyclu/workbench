...
export const EXTERNAL_ACTION_COMMANDS: readonly AuthorizationRule[] = [
  ...
  { id: 'pr_create', description: 'Push the named branch if needed and create or open the named pull request', pattern: /\b(?:create|open|raise|file|submit)\s+(?:(?:a|an|the|another)\s+)?(?:new\s+)?(?:draft\s+)?(?:github\s+)?(?:pull request|pr)\b/i },
