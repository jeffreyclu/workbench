...
export function createConversationRouter({ repository, database, capabilities, admin }: RouteContext) {
  ...
  router.post('/api/shared/messages', (request, response) => {
    ...
    const message = repository.createSharedMessage('jeffrey', input.body, agents.length ? 'queued' : 'completed', input.conversationId, attachments, input.dispatchTo, input.executionProfile, input.accountProfile ?? null, null, agents.length ? input.executionKind ?? 'execute' : null);
