...
/**
 * Adds Google Docs-style comments without owning the artifact's content. Each
 * table row gets a small inline control; the selected row's thread lives in a
 * fixed side rail and is identified by a deterministic table/row selector.
 */
function renderCommentingLayer(feedback: NonNullable<ArtifactPageOptions['feedback']>, version: number): string {
  ...
  return `<style data-wb-commenting-style>.wb-row-comment-button{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;margin-left:8px;padding:0;border:1px solid rgba(128,128,128,.45);border-radius:999px;background:#20201d;color:#d7d6cf;vertical-align:middle;cursor:pointer}.wb-row-comment-button:hover,.wb-row-comment-button:focus-visible{border-color:#c6f432;color:#c6f432;outline:none}.wb-commented-row>th,.wb-commented-row>td{background:rgba(198,244,50,.08)}.wb-comment-selected>th,.wb-comment-selected>td{box-shadow:inset 0 2px #c6f432,inset 0 -2px #c6f432}.wb-comment-rail{position:fixed;z-index:20;top:24px;right:24px;width:min(360px,calc(100vw - 48px));max-height:calc(100vh - 48px);overflow:auto;padding:18px;background:#191918;color:#d7d6cf;border:1px solid #44443d;border-radius:12px;box-shadow:0 16px 48px rgba(0,0,0,.35);font:14px/1.45 ui-sans-serif,system-ui,-apple-system,sans-serif}.wb-comment-rail[hidden]{display:none}.wb-comment-rail h2{margin:0;color:#f4f3ed;font-size:17px}.wb-comment-row-label{margin:5px 0 16px;color:#aaa99f;font-size:12px}.wb-comment-close{float:right;border:0;background:none;color:#aaa99f;font:inherit;cursor:pointer}.wb-comment-thread{display:grid;gap:10px;margin:0 0 16px}.wb-comment-item{padding:10px;border:1px solid #353530;border-radius:8px;background:#10100f}.wb-comment-item p{margin:5px 0 0;white-space:pre-wrap}.wb-comment-empty{margin:0;color:#aaa99f}.wb-comment-form{display:grid;gap:8px}.wb-comment-form input,.wb-comment-form textarea{width:100%;padding:9px 11px;border:1px solid #4a4a43;border-radius:7px;background:#10100f;color:inherit;font:inherit}.wb-comment-form button{justify-self:start;padding:9px 13px;border:0;border-radius:7px;background:#c6f432;color:#111;font:inherit;font-weight:700;cursor:pointer}.wb-comment-form button:disabled{opacity:.6;cursor:wait}.wb-comment-status{min-height:20px;margin:0;color:#aaa99f;font-size:12px}@media(max-width:700px){.wb-comment-rail{top:auto;right:12px;bottom:12px;width:calc(100vw - 24px);max-height:68vh}.wb-row-comment-button{width:36px;height:36px;margin:4px 0 4px 8px}}</style><aside class="wb-comment-rail" id="wb-comment-rail" aria-label="Comments" hidden><button class="wb-comment-close" type="button" aria-label="Close comments">Close</button><h2>Comments</h2><p class="wb-comment-row-label" id="wb-comment-row-label">Select a row to comment.</p><div class="wb-comment-thread" id="wb-comment-thread"></div><form class="wb-comment-form" id="wb-comment-form"><label for="wb-comment-name">Your name</label><input id="wb-comment-name" name="author" placeholder="Your name" maxlength="80" autocomplete="name"><label for="wb-comment-body">Comment</label><textarea id="wb-comment-body" name="body" rows="4" placeholder="Leave a comment on this row" maxlength="5000" required></textarea><button type="submit">Comment</button><p class="wb-comment-status" id="wb-comment-status" role="status"></p></form></aside><script data-wb-commenting>(function(){var endpoint=${JSON.stringify(endpoint)};var version=${version};var rail=document.getElementById('wb-comment-rail');var form=document.getElementById('wb-comment-form');var thread=document.getElementById('wb-comment-thread');var rowLabel=document.getElementById('wb-comment-row-label');var status=document.getElementById('wb-comment-status');var selected=null;var comments=[];function esc(value){var node=document.createElement('span');node.textContent=value;return node.innerHTML}function anchorFor(row){var table=row.closest('table');var tables=Array.prototype.slice.call(document.querySelectorAll('main table'));var tableIndex=tables.indexOf(table)+1;var rows=Array.prototype.slice.call(table.querySelectorAll('tr'));return 'main table:nth-of-type('+tableIndex+') tr:nth-of-type('+(rows.indexOf(row)+1)+')'}function select(row){if(selected)selected.classList.remove('wb-comment-selected');selected=row;row.classList.add('wb-comment-selected');rail.hidden=false;rowLabel.textContent='Selected row: '+Array.prototype.slice.call(row.cells).map(function(cell){return cell.textContent.trim()}).filter(Boolean).join(' · ').slice(0,140);render()}function render(){thread.innerHTML='';if(!selected)return;var anchor=anchorFor(selected);var matches=comments.filter(function(comment){return comment.anchor===anchor});if(!matches.length){thread.innerHTML='<p class="wb-comment-empty">No comments on this row yet.</p>';return}matches.forEach(function(comment){var item=document.createElement('article');item.className='wb-comment-item';item.innerHTML='<strong>'+esc(comment.author)+'</strong><p>'+esc(comment.body)+'</p>';thread.appendChild(item)})}function load(){fetch(endpoint).then(function(response){if(!response.ok)throw new Error();return response.json()}).then(function(payload){comments=payload.comments||[];document.querySelectorAll('main table tr').forEach(function(row){if(comments.some(function(comment){return comment.anchor===anchorFor(row)&&!comment.resolvedAt}))row.classList.add('wb-commented-row')});render()}).catch(function(){status.textContent='Could not load comments.'})}document.querySelectorAll('main table tr').forEach(function(row){var cell=row.cells[row.cells.length-1];if(!cell)return;var button=document.createElement('button');button.type='button';button.className='wb-row-comment-button';button.setAttribute('aria-label','Comment on this row');button.title='Comment on this row';button.innerHTML='&#128172;';button.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();select(row)});cell.appendChild(button)});document.querySelector('.wb-comment-close').addEventListener('click',function(){rail.hidden=true;if(selected)selected.classList.remove('wb-comment-selected');selected=null});form.addEventListener('submit',function(event){event.preventDefault();if(!selected)return;var body=document.getElementById('wb-comment-body').value.trim();if(!body)return;var button=form.querySelector('button');button.disabled=true;status.textContent='Sending…';fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({author:document.getElementById('wb-comment-name').value.trim()||'Coworker',body:body,version:version,anchor:anchorFor(selected)})}).then(function(response){if(!response.ok)throw new Error();return response.json()}).then(function(payload){comments.push(payload.comment);selected.classList.add('wb-commented-row');document.getElementById('wb-comment-body').value='';status.textContent='Comment added.';render()}).catch(function(){status.textContent='Could not send that. Try again.'}).finally(function(){button.disabled=false})});load()})();</script>`;
}
