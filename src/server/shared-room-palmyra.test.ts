...
describe('Palmyra as a conversation provider', () => {
  ...
  it('does not add Palmyra to a Codex conversation turn', async () => {
    const database = openDatabase(':memory:');
    const repository = new WorkItemRepository(database);
    const conversation = repository.createConversation('Codex only', null);
    ...
    try {
      const replies = dispatchNextSharedTurn(repository, conversation.id);
      expect(replies.map((reply) => reply.author)).toEqual(['codex']);
