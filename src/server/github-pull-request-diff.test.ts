...
describe('GitHub pull-request diffs', () => {
  ...
  it('returns one page at a time and exposes a next page for large pull requests', async () => {
    ...
    const fetchImpl = vi.fn(async (input: string | URL | Request) => {
      const endpoint = String(input);
      if (endpoint.endsWith('/pulls/24')) return new Response(JSON.stringify({ html_url: 'https://github.com/writer/workbench/pull/24', title: 'Large', number: 24, base: { ref: 'main' }, head: { ref: 'feature/large', sha: 'b'.repeat(40) }, changed_files: 250, additions: 250, deletions: 0 }), { status: 200 });
  ...
  it('loads image bytes through the authenticated GitHub API for private pull requests', async () => {
    const fetchImpl = vi.fn(async (input: string | URL | Request) => {
      const endpoint = String(input);
      if (endpoint.endsWith('/pulls/24')) return new Response(JSON.stringify({ html_url: 'https://github.com/writer/workbench/pull/24', title: 'Image', number: 24, base: { ref: 'main' }, head: { ref: 'feature/image', sha: 'c'.repeat(40) }, changed_files: 1, additions: 0, deletions: 0 }), { status: 200 });
  ...
  it('uses the head SHA, rather than the mutable head ref, as the review revision across force-pushes', async () => {
    const headRef = 'feature/review';
    const headShas = ['1'.repeat(40), '2'.repeat(40)];
    let request = 0;
    const fetchImpl = vi.fn(async (input: string | URL | Request) => {
      const endpoint = String(input);
      if (endpoint.endsWith('/pulls/24')) {
        const sha = headShas[request++];
        return new Response(JSON.stringify({ html_url: 'https://github.com/writer/workbench/pull/24', title: 'Force pushed', number: 24, base: { ref: 'main' }, head: { ref: headRef, sha }, changed_files: 1, additions: 1, deletions: 0 }), { status: 200 });
      }
      return new Response(JSON.stringify([{ filename: 'src/review.ts', status: 'modified', additions: 1, deletions: 0, patch: '@@ -1 +1 @@\n-old\n+new' }]), { status: 200 });
    });

    const first = await getGitHubPullRequestDiff('https://github.com/writer/workbench/pull/24', { token: 'secret', fetchForPolicy: () => fetchImpl as typeof fetch });
    const second = await getGitHubPullRequestDiff('https://github.com/writer/workbench/pull/24', { token: 'secret', fetchForPolicy: () => fetchImpl as typeof fetch });

    expect(first.headRef).toBe(second.headRef);
    expect(first.headSha).toBe(headShas[0]);
    expect(second.headSha).toBe(headShas[1]);
    expect(first.headSha).not.toBe(second.headSha);
    expect(first.revision).toBe(first.headSha);
    expect(second.revision).toBe(second.headSha);
  });
