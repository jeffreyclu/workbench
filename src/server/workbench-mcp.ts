...
export function createWorkbenchMcpServer(repository: WorkItemRepository, admin: WorkbenchAdminActions): McpServer {
  const server = new McpServer({ name: 'workbench', version: '1.0.0' }, {
    instructions: [
      ...
      'Finite commands expected to run longer than 90 seconds and explicitly requested temporary dev services must use start_managed_command and inspect_managed_command. Workbench writes live output and status to disk, reuses a stable keyed job after a provider retry, and can stop the complete process group. For a service, inspect with waitMs 0 until ready, run its dependent work, then stop_managed_command before the turn ends. Inspect an existing keyed job before starting over.',
  ...
  server.registerTool('start_managed_command', {
    title: 'Start or resume a tracked long-running command or temporary service',
    description: 'Starts one local command outside the provider shell timeout. Stdout, stderr, PID, status, and exit code are written to disk. Pass the raw command without tail or tee because Workbench captures output directly. A stable key reopens the same job after an agent retry instead of rerunning it; set restart only for an intentional new attempt. Temporary dev servers and watchers are allowed only when the current request requires them: inspect with waitMs 0 until ready, run the dependent work, then call stop_managed_command before the turn ends.',
    inputSchema: {
