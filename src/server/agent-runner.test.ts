...
Question or requested change: Replace this hook with the established memoized helper.`)).toBe('execute');
  ...
  it('keeps automatic durable retrieval bounded to historically dependent runs', () => {
    ...
    expect(RUNNER_SYSTEM_CONTRACT).toContain('explicitly asks you to be verbose');
  });
  ...
  it('rejects uninvestigated requests for evidence as a harness violation', () => {
    ...
    expect(hasDeferredExecutionResponse('1. Fix the route.\n2. Run the tests.')).toBe(true);
    expect(hasDeferredExecutionResponse('Fixed the route and ran the focused test.')).toBe(false);
  });
