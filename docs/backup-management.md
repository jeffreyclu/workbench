...
1. Snapshots both live SQLite databases (`data/workbench.db` and `data/workbench-audit.db`) with `VACUUM INTO` into `data/backups/`.
2. Redacts `source_connections.settings_json` (Atlassian, GitHub, Slack, Figma tokens) to `{"redacted":true}`.
3. Compresses both snapshots and splits them into 90 MB chunks (`latest.db.gz.part0000` and `latest-audit.db.gz.part0000`, ...), pushed to the private GitHub repo [`jeffreyclu/workbench-backups`](https://github.com/jeffreyclu/workbench-backups) — separate from the public `workbench` repo. GitHub hard-rejects any single blob over 100 MB; chunking keeps every pushed file under that limit no matter how large either database grows.
...
```
or browse github.com/jeffreyclu/workbench-backups — each push is a commit, so history = restore points. Reassemble and decompress both files before restoring: `cat latest.db.gz.part* > latest.db.gz && gzip -dk latest.db.gz`, then repeat with `latest-audit.db.gz.part*`.
