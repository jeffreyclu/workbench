...
- `frontend-reviewer` is the only authoritative code-review persona and the only entry point for review executions. Its review is read-only and runs five separate passes in order: correctness/readability, performance/scaling, conventions/existing patterns, UX issues/bugs, and security. Every pass reports coverage, every finding is blocking or non-blocking, and testing/runtime validation remain separate executables.
